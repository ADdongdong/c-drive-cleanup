...
- **用户个人静态文件** → 只报告，不删不自动迁（D类：桌面/文档/图片/音乐/视频/下载）

**D类处理规范**（用户个人数据，红线最多）：
- 个人文件**绝不自动删除、绝不自动移动**——只报告位置和大小，建议用户自己搬
- 已知文件夹（桌面/文档等）迁到数据盘的**官方法**：右键文件夹 → 属性 → 位置 → 移动（Windows 自动搬移并改注册表，所有软件路径感知，优于 junction）
- junction 方式对已知文件夹也可行，但注意：OneDrive 备份重定向可能冲突；微信/QQ 的默认保存位置常指向这些目录，迁移后要同步改软件设置
- 检测用 `SHGetKnownFolderPath` API（注册表 `User Shell Folders` 可能为空，不可依赖）
